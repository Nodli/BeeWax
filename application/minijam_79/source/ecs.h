...
struct Archetype{
    ...
    u32 type_offsets[archetype_max_types - 1u];
};
...
u32 add_archetype(Manager& manager){
    if(manager.free_index.size()){
        u32 index = manager.free_index[manager.free_index.size()];
...
void remove_archetype(Manager& manager, u32 index){
    ...
    assert(manager.archetypes[index].ntypes == 0u);
    assert(manager.storage[index].chunks.size() == 0u);
...
void update_archetype_metadata(const Manager& manager, Archetype& archetype, u32 start_index){
    ...
    size_t type_offset = manager.types[archetype.type_IDs[0u]].bytesize;

    for(u32 itype = 1u; itype != archetype.ntypes; ++itype){
        const Type_Metadata& meta = manager.types[archetype.type_IDs[itype]];

        type_offset = align_offset_next(type_offset, meta.alignment);
        archetype.type_offsets[itype - 1u] = type_offset;
...
s32 add_type(const Manager& manager, Archetype& archetype, u32 type_ID){
    ...
    if(archetype.type_IDs[index] == type_ID) return -1;
...
Entity add_component(Manager& manager, Entity entity, u32 type_ID){
    ...
    if(type_insert != -1){
        ...
        if(type_index){
            memcpy(new_ptr, ptr, arch.type_offsets[type_index - 1u]);
        }
        ...
        if(type_index != arch.ntypes){
            ...
            memcpy((u8*)new_ptr + new_ptr_offset, (u8*)ptr + ptr_offset, arch.bytesize - ptr_offset);
        }
...
Entity remove_component(Manager& manager, Entity entity, u32 type_ID){
    ...
    if(type_insert != -1){
        ...
        if(type_index != arch.ntypes - 1u){
            ...
            u32 new_ptr_offset = arch.type_offsets[type_index - 1u];
