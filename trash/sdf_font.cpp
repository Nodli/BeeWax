...
Font_Asset font_asset_from_ttf_file(const File_Path& path,
    ...
    array<BEEWAX_INTERNAL::stbtt_glyph_data> glyph_data;
...
vec2 Font_Renderer::render_string(Font_Asset* asset, const char* string, vec2 baseline, float font_size){
    ...
    for(u32 icodepoint = 0u; icodepoint != string_size; ++icodepoint){
        ...
        ivec2 iquad_min = {
            (s32)(new_baseline.x + (float)cp_info->quad_offset_x * relative_font_scale),
            (s32)(new_baseline.y + (float)cp_info->quad_offset_y * relative_font_scale)
        };
        ivec2 iquad_max = {
            (s32)((float)iquad_min.x + (float)cp_info->quad_width * relative_font_scale),
            (s32)((float)iquad_min.y + (float)cp_info->quad_height * relative_font_scale)
        };
