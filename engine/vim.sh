...
vim -c ":nnoremap <F5> :wa<CR> :!${executable_path}<CR>"                                                                                                                                                                                    \
    -c ":nnoremap <F6> :!<CR>"                                                                                                                                                                                                              \
    -c ":autocmd VimEnter * if filereadable(\"${session_path}\") | :source ${session_path} | :source ${vimrc_path} | else | :args **/*.h **/*.cpp **/*.inl **/*.txt | :set filetype=cpp | :source ${vimrc_path} | :b todo.txt | endif"      \
