...
static inline void setup_perlin_noise_2D(const float x, const float y,
    ...
    hash = BEEWAX_INTERNAL::noise_hash(*(u32*)&hash_data[0u]);
    hash = hash_combine(hash, BEEWAX_INTERNAL::noise_hash(*(u32*)&hash_data[1u]));
...
float simplex_noise(const float x, const float y){
    ...
    if(weight_origin > 0.f){
        ...
        u32 hash_origin = BEEWAX_INTERNAL::noise_hash(*(u32*)&hash_data_origin[0u]);
        hash_origin = hash_combine(hash_origin, BEEWAX_INTERNAL::noise_hash(*(u32*)&hash_data_origin[1u]));
    ...
    if(weight_corner > 0.f){
        ...
        u32 hash_corner = BEEWAX_INTERNAL::noise_hash(*(u32*)&hash_data_corner[0u]);
        hash_corner = hash_combine(hash_corner, BEEWAX_INTERNAL::noise_hash(*(u32*)&hash_data_corner[1u]));
    ...
    if(weight_opposite > 0.f){
        ...
        u32 hash_opposite = BEEWAX_INTERNAL::noise_hash(*(u32*)&hash_data_opposite[0u]);
        hash_opposite = hash_combine(hash_opposite, BEEWAX_INTERNAL::noise_hash(*(u32*)&hash_data_opposite[1u]));
