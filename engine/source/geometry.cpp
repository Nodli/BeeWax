...
void Geometry_Mesh_3D::create(u32 i_nvertices, u32 i_nindices){
    nvertices = i_nvertices;
    nindices = i_nindices;

    size_t bytesize = 2u * nvertices * sizeof(vec3) + nindices * sizeof(u16);
    void* ptr = bw_malloc(bytesize);
...
void Geometry_Mesh_3D::destroy(){
    bw_free(positions);
}

Geometry_Mesh_3D generate_sphere_uv(vec3 position, float radius, u32 nlong, u32 nlat){
    assert(nlat > 0u && nlong > 2u);
    u32 nvertices = 2u + nlat * nlong;
    u32 nindices = 3u * (nlong + nlong + (nlat - 1u) * 2u * nlong);

    Geometry_Mesh_3D mesh;
    mesh.create(nvertices, nindices);
...
Geometry_Mesh_3D generate_cube(vec3 position, float size, u32 ntess){
    return generate_cuboid(position, {size, size, size}, ntess);
}

Geometry_Mesh_3D generate_cuboid(vec3 position, vec3 size, u32 ntess){
...
void triangulation_2D_v0(u32 nvertices, vec2* vertices, u32* nindices){
    ...
    BEEWAX_INTERNAL::trig_2D_LLE* connectivity_LL = (BEEWAX_INTERNAL::trig_2D_LLE*)bw_malloc(nvertices * sizeof(BEEWAX_INTERNAL::trig_2D_LLE));
    DEFER{
        bw_free((void*)connectivity_LL);
    };
...
void triangulation_2D_v1(u32 nvertices, vec2* vertices, u32* nindices){
    ...
    BEEWAX_INTERNAL::trig_2D_LLE* connectivity_LL = (BEEWAX_INTERNAL::trig_2D_LLE*)bw_malloc(nvertices * sizeof(BEEWAX_INTERNAL::trig_2D_LLE));
    u32* reflex_LL = (u32*)bw_malloc(nvertices * sizeof(u32));
    memset(reflex_LL, 0xFF, nvertices * sizeof(u32));
    DEFER{
        bw_free((void*)reflex_LL);
        bw_free((void*)connectivity_LL);
    };
