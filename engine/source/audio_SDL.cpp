...
void Audio_Player_SDL::create(){
    ...
    required_spec.callback = BEEWAX_INTERNAL::audio_callback;
...
Audio_Channel Audio_Player_SDL::start(const Audio_Asset* asset, Audio_Channel_State state){
    assert(state == Audio_Channel_State::PLAY || state == Audio_Channel_State::LOOP);
...
void Audio_Player_SDL::stop(const Audio_Channel& channel){
    ...
    && atomic_get(&(mixer.audio_info[index].state)) > Audio_Channel_State::STOP){

        atomic_set(&(mixer.audio_info[index].state), Audio_Channel_State::STOP);
    }
}
    ...
    static void audio_callback(void* user_ptr, u8* out_stream, s32 out_stream_size){
        ...
        for(u32 iinfo = 0u; iinfo != BEEWAX_INTERNAL::audio_info_capacity; ++iinfo){
            ...
            if(audio_state == Audio_Channel_State::LOOP && info.asset->nsamples > 0u){
                ...
                // NOTE(hugo): no fallthrough
                continue;
            }

            if(audio_state == Audio_Channel_State::PLAY){
                u32 mix_cursor = 0u;
                u32 asset_cursor = info.cursor;
