...
int main(int argc, char* argv[]){
    ...
    particle_emitter.theta_min_range[1] = PI * 2.f;
	...
	while(running){
        ...
        if(true)
        {
            const char* string = "Asobisof";

            Font_Asset* font = asset.get_font("roboto");
            Text_Box tb = compute_string_text_box(font, string, {0.f, 0.f}, 250.f);
            Layout_Box lb = {{0.f, 0.f}, {(float)window.width, (float)window.height}};

            center_vertical(tb, lb);
            center_horizontal(tb, lb);

            vec2 baseline_end = font_renderer.render_string(font, string, tb.baseline, 250.f);

            // NOTE(hugo): underline
            float underline_height = 10.f;
            Vertex_Batch_ID vertex_batch = renderer.get_vertex_batch(xyrgba, PRIMITIVE_TRIANGLE_STRIP);
            vertex_xyrgba* vertices = (vertex_xyrgba*)renderer.get_vertices(vertex_batch, 4u);
            ivec2 imin = {(int)tb.baseline.x, (int)(tb.baseline.y - 2.f * underline_height)};
            ivec2 imax = {(int)baseline_end.x, (int)(tb.baseline.y - underline_height)};
        ...
        // NOTE(hugo): particles
        if(particle_emitter.storage.size > 0u)
        {
